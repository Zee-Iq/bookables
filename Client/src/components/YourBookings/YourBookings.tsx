...
export default function YourBookings() {
  ...
  useEffect(() => {
    ...
    if (
      loggedInUser &&
      loggedInUser.roles.includes("tenant") &&
      (location.state as any)?.fromDate &&
      (location.state as any)?.toDate &&
      (location.state as any)?.bookableId
    ) {
      dispatch(
        createReservation({
  ...
  if (!loggedInUser) {
    return (
      <Navigate
        to="/login"
        state={{ from: location, reservationData: location.state }}
      />
    );
            ...
            >
              <TableCell>
                <p style={{ fontWeight: "bold" }}>
                  {reservation.bookable.spaceId.name}
                </p>
              ...
              <TableCell align="right">
                <p>
                  {" "}
                  from:{" "}
                  {reservation.from.toString().slice(0, -8).replace("T", " ")}
                </p>
                <p>
                  {" "}
                  To: {reservation.to.toString().slice(0, -8).replace("T", " ")}
                </p>
              </TableCell>
              <TableCell align="right">
                <p>{reservation.bookable.name}</p>
                <p>Type: {reservation.bookable.type}</p>
              </TableCell>
              <TableCell align="right">
                {Number(
                  Math.abs(
                    moment(reservation.from).diff(
                      moment(reservation.to),
                      "hours",
                      true
                    )
                  ) *
                    (reservation.bookable.hourlyRate / 100)
                ).toFixed(2)}
                €
              </TableCell>
              <TableCell align="right">
